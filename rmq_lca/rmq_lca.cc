...
template<typename T, bool maximum_node = false>
struct RMQ {
    ...
    // Note: when `values[a] == values[b]`, return b
    int better_index(int a, int b) const {
        return (maximum_node ? values[b] < values[a] : values[a] < values[b]) ? a : b;
    }
    ...
    // Note: breaks ties by choosing largest index.
...
struct LCA {
    ...
    int get_kth_ancestor(int a, int k) const {
        ...
        int target = depth[a] - k;

        while(heavy_root_depth[a] > target)
            a = heavy_root_parent[a];
        
        return tour_list[tour_start[a] - (depth[a] - target)];
    }
